...
Устанавливает значение по пути в dot-нотации (`a.b.c`). Промежуточны�� массивы создаются автоматически.
...
Возвращает ��ервый элемент.

Режимы:
- Без `$fn`: вернёт первый элемент коллекции (по текущему порядку) или `$default`, е��ли коллекция пустая.
...
Параметры:
- `$prepend` — префикс дл�� всех ключей (опционально).
...
- `$options`:
  ...
  - `array`: расширенные настройки:
    ...
    - `list` (`replace|append|append_unique`, по умолчанию `replace`) — страте��ия для списков (когда обе стороны — списки).
