...
final class ArrayHelper
{
    ...
    public static function hasPath(array $items, string $path): bool
    {
        if (str_contains($path, '*')) {
            ...
            return array_any($matches, fn ($match) => $match['present']);
    ...
    {
        if ($callback === null) {
            return array_all($array, fn ($value) => $value);
        }
    ...
    public static function flatten(array $array, int|float $depth = INF): array
    {
        ...
        $flatten = static function (array $items, int|float $currentDepth) use (&$flatten, &$result): void {
    ...
    {
        if ($callback === null) {
            return array_any($array, fn ($value) => $value);
        }
    ...
    public static function toCssClasses(array|string $classes): string
    {
        ...
        return implode(' ', array_values(array_filter($result, static fn ($value) => $value !== '')));
    }
